...
\end{flalign}
%
The sampling time is chosen to be \SI{0,01}{s}. This decision is based on the results from \appref{app:TimingProgram}, where it can be seen that the code takes  around \SI{0,008}{s}. To avoid timing issues a frequency of \SI{100}{Hz} is chosen and results in the sampling time previously mentioned.
...
\begin{figure}[H]
  ...
  \caption{Bode plot of the open loop system with the continuous controller (in blue), ZOH discretized controller (in orange), Tustin discretized controller (in yellow) and pre-warped Tustin discretized controller (in purple).}
