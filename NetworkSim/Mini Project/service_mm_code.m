function [exectime, data] = service_mm_code(seg,data)

switch seg
  case 1 
    dataQ=ttGetData('generator_mm_task'); % Read what is in the queue
    dataT=ttGetData('token_mm_task');     % Read what is in the bucket
    ...
    while(transmission==1)
        ...
        if(dataT.tokens>0)                  % If there are tokens
            msg = ttTryFetch('buffer_mm');  % Message is extracted from buffer
            if(dataQ.Q>0)                               % If there are packets in the queue 
                dataQ.Q=dataQ.Q-1;                      % Reduce number of packets in the queue
                ttSetData('generator_mm_task',dataQ);   % Change data   
                dataT.tokens=dataT.tokens-1;            % Reduce number of tokens
                ttSetData('token_mm_task',dataT);       % Change number of tokens
                transmission=1;                         % We enter the loop again   
                data.seq=data.seq+1;
                 
                ttSendMsg(1, msg, 1400*8, 3);     % send msg to gateway (node 1)
