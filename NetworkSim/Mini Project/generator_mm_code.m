function [exectime, data] = generator_mm_code(seg,data)
switch seg
  case 1 
    ...
    if(data.Q<data.BufferSize) % If queue lenght is smaller than L
        ...
        msg.seq=data.seq;   
        ttTryPost('buffer_mm', msg);
        ttCreateJob('service_mm_task');   % Create job for service task ("A message has arrived")
    end
    ...
    ttCreateJob('generator_mm_task', ttCurrentTime+time); 
    exectime=0;
    %%%%%% 
  case 2
  exectime = -1;
